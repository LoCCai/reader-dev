...
8. **camoufox 求解器 cookie 外传**（m12）：验证码求解时当前书源 cookie 会随请求发送至 `READER_CAMOUFOX_URL`。默认 `127.0.0.1` 无风险；**若配置为远端地址，cookie 将明文发给该第三方**——务必仅指向自控实例，或经加密隧道（VPN/Tailscale 等），并在代理层终止 TLS。
